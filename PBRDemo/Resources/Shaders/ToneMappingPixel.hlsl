...
float3 ApplyHue(float3 color, float hue_in) {
	...
	float YPrime = dot(float4(color, 1.0f), kRGBToYPrime);
	float I = dot(float4(color, 1.0f), kRGBToI);
	float Q = dot(float4(color, 1.0f), kRGBToQ);
...
float4 main(VertexOutputType input) : SV_Target
{
	float gamma = 2.0f;
	float exposure = 1.0f;

	float4 sampledValue = pow(hdrTexture.Sample(SampleType, input.tex), gamma);

	float3 result;

	result = AllTonemappingAlgorithms(sampledValue.xyz, input.tex.x + input.tex.y, exposure, gamma);
